...
<section id="container">
    <div id="content" class="service_notice detail">
        <section class="cont_header">
            ...
            <span class="page_path">HOME &gt; 서비스 관리 &gt; 공지사항 수정</span>
        </section>
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                            ...
                            <div class="file_list" v-for="(list, index) in fileList" :key="index">
                                <p>{{ list.split('/')[3] }} <button @click="fileDelete(list, index)">삭제</button></p>
                            </div>
                            <div class="add_file_list" v-for="(addList, index) in addFileList" :key="'A'+index">
                                <p>{{ addList.name }} <button @click="addFileDelete(index)">삭제</button></p>
                            </div>
                        </td>
                    </tr>
                </tbody>
...
export default{
    ...
    created(){
        ...
        .then(data=>{
            ...
            $('#dateFrom').datepicker("setDate", this.formData.notice_start_dt);
            $('#dateTo').datepicker("setDate", this.formData.notice_end_dt);
            this.updator_id=this.$store.getters.getAdminInfo.admin_user_id;
            this.updator_name=this.$store.getters.getAdminInfo.admin_name;

            if(this.formData.add_file1_url != null){
                this.fileUrlLoad(this.formData.add_file1_url);
            }
            if(this.formData.add_file2_url != null){
                this.fileUrlLoad(this.formData.add_file2_url);
            }
            if(this.formData.add_file3_url != null){
                this.fileUrlLoad(this.formData.add_file3_url);
            }
            if(this.formData.add_file4_url != null){
                this.fileUrlLoad(this.formData.add_file4_url);
            }
            if(this.formData.add_file5_url != null){
                this.fileUrlLoad(this.formData.add_file5_url);
            }
        })
        .catch(err=>{
    ...
    computed:{
        isAuthor(){
            if(this.$store.getters.getAuthType === "1"  || this.$store.getters.getAuthType === "0" || this.formData.creator_id===this.$store.getters.getAdminInfo.admin_user_id){
                return true;
            } else{
                return false;
            }
    ...
    methods:{
        ...
        fileUpload(event){
            for(let i=0; i<event.target.files.length; i++){
                if(this.fileList.length + this.addFileList.length < 5 && event.target.files[i].size <= 10485760){
                    this.addFileList.push(event.target.files[i]);
                }
            }
        },
        fileDelete(list, index){
            this.fileList.splice(index, 1);
            this.deleteFileList.push(list);
        },
        addFileDelete(index){
            this.addFileList.splice(index, 1);
        },
        cancelClick(){
            this.$router.push({name:'servicenoticedetail', params:{notice_id  : this.notice_id }})
        },
        saveClick(){
            if(this.formData.notice_title.length===0){
                ...
                form.append("updator_name", this.updator_name);
                form.append("updator_id", this.updator_id);
                for(let i=0; i<this.fileList.length; i++) {
                    form.append("fileList", this.fileList[i]);
                }
                for(let i=0; i<this.addFileList.length; i++) {
                    form.append("addFileList", this.addFileList[i]);
                }
                for(let i=0; i<this.deleteFileList.length; i++) {
                    form.append("deleteFileList", this.deleteFileList[i]);
                }

                CSNetwork.request_notice_update(form, {
                    headers: {
